...
| 7 | Mutation-test / robustness policy | **OPEN — policy authored [Δ]** (was "OPEN"/undecided; NOT cleared) | **`docs/release/gate7-robustness-policy-00.md`** (WI-RELEASE-G7-ROBUSTNESS-POLICY-00): the v1 local-first robustness policy — six closure criteria (crash/data-loss tolerance; local-first persistence/recovery; offline/online reconciliation = **N/A v1**; audit/event integrity under failure; failure-mode handling; sufficient-evidence-to-clear) mapped to existing evidence (`auditChain.ts`, `better-sqlite3` WAL/synchronous + startup schema-integrity check per brief §14, stable `CaseBoxPersistenceError`/`OcrQueueError` codes, gate-5 graceful shutdown); dependencies mapped to gates 6/12/14/15/5 (not cleared) | **Gate 7 stays NOT CLEARED — BLOCKED on user risk-acceptance + the crash-recovery drill (if required by D-G7-2).** The policy **recommends** accept-v1-without-mutation-testing + a documented crash-recovery drill (a separate future WI; mutation/fuzz deferred post-v1) but **surfaces the risk-acceptance as a USER decision** (D-G7-1 accept-without-mutation-testing?; D-G7-2 require the crash-recovery drill?). Clear depends on those user decisions + the drill (if required); a gate policy does NOT imply go-live. |
