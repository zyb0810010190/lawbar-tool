...
| 3 | Client surface (Mac desktop app) | **PARTIAL [Δ]** — was "NOT STARTED"; corrected by evidence inventory | **12 renderer screens** (`renderer/screens/`: listMatters/createMatter/archiveMatter/viewMatter + viewMatter{Audit,Deadlines,DocketProposals,Documents,Facts,Links,T3Catalog} + auditEventLabels); **26 case-box IPC channels** (`src/caseBox/handlerShared.ts`); electron `main.ts` + `preload.mts` present; **forms-T3 证据目录及说明 preview + DOCX export** shipped (`t3*.ts`; channels `casebox:t3:previewCatalog`/`exportDocx`) | **Substantial functional case-box UI is built** — but the app self-declares "Token-compliance fixture only; NOT product UI; NOT signed; NOT for distribution" (`apps/lawbar-desktop/package.json:6`). Remaining: product-grade hardening + the overdue-deadline dashboard-banner check (brief §10) + release posture. Depends on gate 4 (STOP-AND-ASK). |
...
| 5 | All package tests pass | **PARTIAL [Δ]** | contract 455/0, persistence 573+273+288/0 (this refresh); desktop 791/0 (last verified @ PR #175 merge; app code unchanged since); **1 pre-existing ocr-worker SIGINT flake** (hot-system load) | Deterministically resolve OR explicitly quarantine the ocr-worker flake (documented-acceptance is NOT sufficient for GO). |
