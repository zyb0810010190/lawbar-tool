...
- **A0.7:** ~~meaningfully green (messy fixtures merged)~~ **PROVISIONAL, not green (corrected 2026-08-12; reason superseded 2026-08-29)** — the 2026-08-12 reason was that the merged messy fixtures asserted box geometry only, with three of five oracles carrying zero `samplePoints`. That is no longer true: D3 populated all five and D2 now refuses a box-only oracle outright. **The verdict is unchanged** — A0.7 remains PROVISIONAL because D5 is open and nothing can prove the gate ran. A10 geometry-derived citations rest on it, so this correction matters to A10's premise. (Marker/custody = separate WI, and its tooling was deleted 2026-08-10.)
