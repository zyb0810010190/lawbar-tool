// Queue adapter types.
//
// Step 10B: the queue backend is a *transport-only* lease-aware seam. It
// owns claims, leases, and receipts; it does NOT write OCR contract
// statuses or results. Persistence ownership lives one layer above (the
// adapter / future coordinator). A BullMQ-backed implementation will
// plug in behind the same interface — nothing here imports a queue
// runtime, Redis, or BullMQ.
 ...
 * coordinator) match on `code`, not on `message`.
 */
