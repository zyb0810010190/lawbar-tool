...
test("impl-parity B6.0: not_implemented surfaces uniformly on SQLite for non-B1..B6 methods", async () => {
  const { sqlite } = makePair();
  let err;
  try { await sqlite.appendDocketEntry({}); } catch (e) { err = e; }
  assert.ok(err);
  assert.equal(err.name, "CaseBoxPersistenceError");
  assert.equal(err.code, "not_implemented");
  assert.match(err.message, /appendDocketEntry/);
});
...
// ---------------------------------------------------------------------------
// B6 facts parity tests (per B6 plan §1.6)
// ---------------------------------------------------------------------------
