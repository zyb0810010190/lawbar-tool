...
test("impl-parity B4.0: not_implemented surfaces uniformly on SQLite for non-B1/B2/B3/B4 methods", async () => {
  const { sqlite } = makePair();
  let err;
  try { await sqlite.appendPrivilegeMarker({}); } catch (e) { err = e; }
  assert.ok(err);
  assert.equal(err.name, "CaseBoxPersistenceError");
  assert.equal(err.code, "not_implemented");
  assert.match(err.message, /appendPrivilegeMarker/);
});
