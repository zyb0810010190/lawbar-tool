...
// scoped to B4's filter pattern per the reviewed B4 plan §1.8.
//
...
// (or `${label}: R5.N` / `${label}: R6.N`). We pass label "Sqlite-B4";
// node:test's `--test-name-pattern` (run via the npm test command)
// filters the run to B4's scope. The B4 conformance pattern is the
// constant `B4_PATTERN` below; B4 is a SUPERSET of B3 (matter +
...
// `(?:\s|$)` lookahead on the `-`).
const B4_PATTERN = /^Sqlite-B4: (?:6\.1\.(?:13a|26-27|[1-9]|1[0-5]|1[6-9]|2[0-8]|29|30|31|3[2-8])|6\.A2\.(?:9a|9b|[1-9]|1[0-9]|2[0-4])|R5\.(?:[1-9]|9b|1[0-4])|R6\.[1-3])(?:\s|$)/;

// Hand-enumerated list of B4 case ids per the reviewed B4 plan §1.8.
// Format matches the harness's `${label}: N <description>` shape.
const B4_EXPECTED_CASE_IDS = [
...
// and does NOT match cases outside B4's scope. We do NOT scan the
...
test("Sqlite-B4 conformance preflight: regex matches exactly the B4 case set", () => {
  ...
  const matched = [];
  for (const id of B4_EXPECTED_CASE_IDS) {
    const synthetic = `Sqlite-B4: ${id} placeholder description`;
    if (B4_PATTERN.test(synthetic)) matched.push(id);
  }
  assert.deepEqual(matched, B4_EXPECTED_CASE_IDS, "B4 regex must match every expected case id");

  // Verify the regex does NOT match cases outside B4's scope (smoke).
  // 6.A2.* is now in B4 scope; B4 stops there. 6.A3+ is B5+.
  for (const outside of ["6.A3.1", "6.A4.1", "6.A5.1", "6.A9.1", "R6.4", "6.A2.25"]) {
    const synthetic = `Sqlite-B4: ${outside} placeholder description`;
    assert.equal(B4_PATTERN.test(synthetic), false, `${outside} must NOT match B4 pattern`);
  }
});
...
runConformance("Sqlite-B4", () => ({ Persistence: SqliteCaseBoxPersistenceTestWrapper }));
