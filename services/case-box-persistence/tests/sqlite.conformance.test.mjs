...
// scoped to B10's filter pattern per the reviewed B11 plan §1.4.
//
...
//
// Filter mechanism (CANONICAL per umbrella §2 + B1..B11 plans §1): the
// shared harness labels every test as `${label}: 6.1.N <description>`
// (or `${label}: R5.N` / `${label}: R6.N`). We pass label "Sqlite-B11";
...
// constant `B11_PATTERN` below; B10 is a SUPERSET of B1..B9 (matter +
...
//
// Preflight assertion (per B1..B11 plans §1.3-§1.6):
...
// `(?:\s|$)` lookahead on the `-`).
const B11_PATTERN = /^Sqlite-B11: (?:6\.1\.(?:13a|26-27|[1-9]|1[0-5]|1[6-9]|2[0-8]|29|30|31|3[2-8])|6\.A2\.(?:9a|9b|[1-9]|1[0-9]|2[0-4])|6\.A3\.(?:A2b|20b|21b|[1-9]|1[0-9]|2[0-7])|6\.A4\.(?:10b|10c|27b|27c|[1-9]|1[0-9]|2[0-8])|6\.A5\.(?:4b|18a|18b|19b|19c|[1-9]|1[0-9]|2[0-9]|3[0-2])|6\.A6\.(?:5b|18b|21b|[1-9]|1[0-9]|2[0-2])|6\.A7\.(?:1b|15b|1|2|3|4|6|7|8|11|12|13|14|15|16|17|18)|6\.A8\.(?:11b|12b|12c|19b|19c|24b|[1-9]|1[0-9]|2[0-4])|6\.A9\.(?:1[0-3]|[1-9])|R5\.(?:[1-9]|9b|1[0-9]|20|2[1-4])|R6\.[1-4])(?:\s|$)/;

// Hand-enumerated list of B10 case ids per the reviewed B11 plan §1.4.
// Format matches the harness's `${label}: N <description>` shape.
const B11_EXPECTED_CASE_IDS = [
  ...
  "6.A8.24", "6.A8.24b",
  "R6.4",
  // B11 replay-safe Once variants (6.A9.* incl. 1..13; no variants)
  // + R5.16/17/18 (appendFactOnce canonical-projection equivalence).
  "6.A9.1", "6.A9.2", "6.A9.3", "6.A9.4", "6.A9.5",
  "6.A9.6", "6.A9.7", "6.A9.8", "6.A9.9",
  "6.A9.10", "6.A9.11", "6.A9.12", "6.A9.13",
  "R5.16", "R5.17", "R5.18",
];
...
test("Sqlite-B11 conformance preflight: regex matches exactly the B6 case set", () => {
  ...
  const matched = [];
  for (const id of B11_EXPECTED_CASE_IDS) {
    const synthetic = `Sqlite-B11: ${id} placeholder description`;
    if (B11_PATTERN.test(synthetic)) matched.push(id);
  }
  assert.deepEqual(matched, B11_EXPECTED_CASE_IDS, "B11 regex must match every expected case id");

  // Verify the regex does NOT match cases outside B6's scope (smoke).
  // 6.A3.* is now in B5 scope; B5 stops there. 6.A4+ is B6+.
  for (const outside of ["6.A7.5", "6.A7.9", "6.A7.10", "6.A7.19", "6.A8.25", "6.A9.14", "6.A10.1", "R6.5"]) {
    const synthetic = `Sqlite-B11: ${outside} placeholder description`;
    assert.equal(B11_PATTERN.test(synthetic), false, `${outside} must NOT match B11 pattern`);
  }
});
...
runConformance("Sqlite-B11", () => ({ Persistence: SqliteCaseBoxPersistenceTestWrapper }));
