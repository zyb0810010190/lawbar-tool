...
// scoped to B5's filter pattern per the reviewed B4 plan §1.8.
//
...
// (or `${label}: R5.N` / `${label}: R6.N`). We pass label "Sqlite-B5";
// node:test's `--test-name-pattern` (run via the npm test command)
// filters the run to B5's scope. The B4 conformance pattern is the
// constant `B5_PATTERN` below; B4 is a SUPERSET of B3 (matter +
...
// `(?:\s|$)` lookahead on the `-`).
const B5_PATTERN = /^Sqlite-B5: (?:6\.1\.(?:13a|26-27|[1-9]|1[0-5]|1[6-9]|2[0-8]|29|30|31|3[2-8])|6\.A2\.(?:9a|9b|[1-9]|1[0-9]|2[0-4])|6\.A3\.(?:A2b|20b|21b|[1-9]|1[0-9]|2[0-7])|R5\.(?:[1-9]|9b|1[0-4])|R6\.[1-3])(?:\s|$)/;

// Hand-enumerated list of B4 case ids per the reviewed B4 plan §1.8.
// Format matches the harness's `${label}: N <description>` shape.
const B5_EXPECTED_CASE_IDS = [
...
// and does NOT match cases outside B5's scope. We do NOT scan the
...
test("Sqlite-B5 conformance preflight: regex matches exactly the B5 case set", () => {
  ...
  const matched = [];
  for (const id of B5_EXPECTED_CASE_IDS) {
    const synthetic = `Sqlite-B5: ${id} placeholder description`;
    if (B5_PATTERN.test(synthetic)) matched.push(id);
  }
  assert.deepEqual(matched, B5_EXPECTED_CASE_IDS, "B5 regex must match every expected case id");

  // Verify the regex does NOT match cases outside B5's scope (smoke).
  // 6.A3.* is now in B5 scope; B5 stops there. 6.A4+ is B6+.
  for (const outside of ["6.A4.1", "6.A5.1", "6.A9.1", "R6.4", "6.A2.25", "6.A3.28"]) {
    const synthetic = `Sqlite-B5: ${outside} placeholder description`;
    assert.equal(B5_PATTERN.test(synthetic), false, `${outside} must NOT match B5 pattern`);
  }
});
...
runConformance("Sqlite-B5", () => ({ Persistence: SqliteCaseBoxPersistenceTestWrapper }));
