...
// (or `${label}: R5.N` / `${label}: R6.N`). We pass label "Sqlite-B8";
...
// constant `B8_PATTERN` below; B7 is a SUPERSET of B1..B6 (matter +
...
// `(?:\s|$)` lookahead on the `-`).
const B8_PATTERN = /^Sqlite-B8: (?:6\.1\.(?:13a|26-27|[1-9]|1[0-5]|1[6-9]|2[0-8]|29|30|31|3[2-8])|6\.A2\.(?:9a|9b|[1-9]|1[0-9]|2[0-4])|6\.A3\.(?:A2b|20b|21b|[1-9]|1[0-9]|2[0-7])|6\.A4\.(?:10b|10c|27b|27c|[1-9]|1[0-9]|2[0-8])|6\.A5\.(?:4b|18a|18b|19b|19c|[1-9]|1[0-9]|2[0-9]|3[0-2])|6\.A6\.(?:5b|18b|21b|[1-9]|1[0-9]|2[0-2])|R5\.(?:[1-9]|9b|1[0-5]|19|20|2[1-4])|R6\.[1-3])(?:\s|$)/;

// Hand-enumerated list of B7 case ids per the reviewed B7 plan §1.6.
// Format matches the harness's `${label}: N <description>` shape.
const B8_EXPECTED_CASE_IDS = [
...
test("Sqlite-B8 conformance preflight: regex matches exactly the B6 case set", () => {
  ...
  const matched = [];
  for (const id of B8_EXPECTED_CASE_IDS) {
    const synthetic = `Sqlite-B8: ${id} placeholder description`;
    if (B8_PATTERN.test(synthetic)) matched.push(id);
  }
  assert.deepEqual(matched, B8_EXPECTED_CASE_IDS, "B8 regex must match every expected case id");

  // Verify the regex does NOT match cases outside B6's scope (smoke).
  // 6.A3.* is now in B5 scope; B5 stops there. 6.A4+ is B6+.
  for (const outside of ["6.A6.23", "6.A7.1", "6.A8.1", "6.A9.1", "R6.4", "R5.16", "R5.17", "R5.18"]) {
    const synthetic = `Sqlite-B8: ${outside} placeholder description`;
    assert.equal(B8_PATTERN.test(synthetic), false, `${outside} must NOT match B8 pattern`);
  }
});
...
runConformance("Sqlite-B8", () => ({ Persistence: SqliteCaseBoxPersistenceTestWrapper }));
