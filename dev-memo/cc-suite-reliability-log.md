...
## 2026-07-07 — batch-audit-230 Layer-B closeout (TIMEOUT -> Path 1 retry BATCH-PASS)
- audit-mrabdorq-3154ly: TIMEOUT (spawnSync codex ETIMEDOUT) on the full-rigor closeout audit prompt at effort medium over the `28402fc..ff40720` window (the gate-18 export/backup-format governance window). Prompt was the standard inlined 57 KB diff (dominated by the gate-18 WI's dense governance prose in `queue.md`). Runner wrote its terminal `status:"failed"` state cleanly (NOT a HARNESS_REAP; the harness auto-backgrounded the wrapping Bash call, but the runner reached its own spawnSync timeout branch and emitted `{status:"failed", error}` with no rawOutput; the completion notification fired correctly). Marker NOT advanced.
- Retry audit-mrach6cg-6xvknj (Path 1, effort medium, IDENTICAL prompt): BATCH-PASS C0 H0 M0 L0, sha c9f4e0ee… (attestation dev-memo/study/2026-07-07-batch-audit-230.md). The identical-prompt retry succeeding confirms a TRANSIENT codex-side timeout (not a prompt-size overrun — batch-228 audited the same ~57 KB governance-window size on first attempt).
- Class: TIMEOUT. Resolution: single Path 1 retry (same prompt, medium effort) succeeded — no prompt narrowing needed, no Path 2/3 fallback. Consistent with the batch-207 precedent that a Path 1 retry clears a transient audit TIMEOUT.

## 2026-07-08 — WI-RELEASE-G3-R3-POLISH-ASSESS-00 governance review-plan (MODEL_API_ERROR -> compact retry READY)
- review-plan attempt 1 (Path 1, FULL ~24 KB WI packet, gpt-5.5/medium): runner stalled ~14 min with no terminal write; the wrapping Bash call was auto-backgrounded. Root cause surfaced on attempt 2 = the Codex model returning `ERROR: Selected model is at capacity`. Class **MODEL_API_ERROR** (not TIMEOUT — no `spawnSync ETIMEDOUT`; the runner was stuck waiting on an at-capacity model). The stuck runner was killed (`pkill codex-runner`); the orphan `running` job (review-plan, pid 60692) was reaped in state.json (`running -> failed`) per the HARNESS_REAP recovery procedure (applied to a stuck/killed runner, same steps). No job artifact.
...
## 2026-08-05 — batch-audit-280 Layer-B closeout (ENOBUFS ×3 -> runner maxBuffer patch -> BATCH-PASS)
- Window `2932b87..d4ebb01` (UI restyle: radius + elevation). Three consecutive Path-1 runner failures, all with **empty rawOutput** and `error: "spawnSync codex ENOBUFS"`: audit-msfjsckb-9ovt6z (high), audit-msfjuntt-fqavxq (high, identical-prompt retry), audit-msfjzu4v-zzvhnb (medium). Prompt was tiny (7.7 KB) — NOT a size/timeout issue.
- **Class: RUNNER_ERROR (new sub-signature = ENOBUFS, distinct from ETIMEDOUT).** Root cause: `runCodexSync` in `codex-runner.mjs` calls `spawnSync("codex", …)` with **no `maxBuffer`** → Node's 1 MB default. Since `--quiet` was dropped for codex-cli ≥0.133 (see the 2026-05 version-mismatch entry), `codex exec` streams verbose session output; a batch audit exceeds 1 MB → `spawnSync` aborts with `ENOBUFS` and writes `{error}` (no rawOutput). Diagnosis ruled out a process/FD leak (0 orphaned `codex exec`/runner procs) and prompt size (7.7 KB failed; larger prompts had succeeded earlier when output stayed <1 MB). Lowering effort high→medium did NOT fix it (verbosity is inherent to `codex exec`, not effort-bound).
- **Fix (Frank-authorized 2026-08-05):** added `maxBuffer: 64 * 1024 * 1024` to the `runCodexSync` spawnSync options in `~/.claude/plugins/cache/xiaolai/cc-suite/0.2.18/scripts/codex-runner.mjs` (backup `*.bak-maxbuffer`; `node --check` OK). Editing under `~/.claude` is an autonomy hard-stop, so it required explicit user authorization; granted for this narrow buffer-only change. Re-run audit-msflxwtk-fhg6vd (Path 1, high, IDENTICAL prompt): **BATCH-PASS C0 H0 M0 L0**, rawOutput 2058 chars, sha ed8c566f… (attestation dev-memo/study/2026-08-05-batch-audit-280.md). Closeout advanced the marker (c5b131f).
- Corroboration: while the runner was down, a Path-2 MCP audit (thread 019fd012, medium) independently returned BATCH-PASS — but Path 2 cannot feed the closeout (no runner job file), so the patched Path 1 was still required.
- **Caveat / follow-up:** the patch lives in the plugin cache — a cc-suite version bump drops it; re-apply if ENOBUFS recurs. Consider codifying ENOBUFS as a formal RUNNER_ERROR sub-class in `.claude/rules/cc-suite.md` §"Timeout / failure classification" via a rule WI (with the echo-sleuth pre-flight), and upstreaming a `maxBuffer` default to cc-suite.
