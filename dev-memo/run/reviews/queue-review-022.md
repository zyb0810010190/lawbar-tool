...
## AMENDMENT-2 re-review (2026-06-07)
Added `apps/lawbar-desktop/tests/casebox-ipc.electron.test.mjs` to WI-BS1 Allowed-files (user-approved) because the BS1 audit (audit-mq4n02kr-7eem2t) surfaced a 2nd collateral breakage: the packaged-Electron IPC test asserts the PRE-FIX leaky createMatter response (lines 161-165: createRes.value.{tenant_id, actor_user_id, external_ocr_authorized, sync_grant_present, llm_extraction_opt_in}). Run by test:ipc-packaged (not BS1's npm-test gate or PR CI); the flip (those fields present -> absent; keep id + name) must live in BS1 so the repo test stays truthful. Re-review review-plan-mq4nacni-jo79x9 returned PASS (no C/H/M): minimal correct amendment; strengthens the boundary check; opt-in flags correctly excluded (non-consumed); no scope broadening. Re-lint PASSED; re-governed content-bound.
