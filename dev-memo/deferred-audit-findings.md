...
| LOC-RVM-1200 | Low | Open (follow-up = a bounded TEST split WI) — `apps/lawbar-desktop/tests/renderer-view-matter.test.mjs` reached EXACTLY 1200 raw LOC during WI-701/WI-703 (loc-guardian hand-written-test fail = raw LOC >= 1200), a pre-existing violation by 0 lines over the boundary. WI-702 review-plan `review-plan-mq11is6s-atrgyr` flagged it (Medium) when WI-702 was about to add deadline tests there. **User-approved deferral (2026-06-05, Option B)**: WI-702's new deadline UI + DST-helper tests go into a NEW dedicated file `apps/lawbar-desktop/tests/renderer-deadline-write.test.mjs`; `renderer-view-matter.test.mjs` is moved to WI-702 Forbidden-files and left UNCHANGED at 1200, so WI-702 adds NO new violation. The pre-existing 1200-LOC condition is a KNOWN, UNCHANGED violation accepted for this batch step only. | future bounded TEST WI: split the deadline-related view tests out of `renderer-view-matter.test.mjs` into a sibling file so the host drops below 1200 (mirrors the `renderer-deadline-urgency.test.mjs` precedent) | YES | open | non-blocking; test-file size only (no behavior impact); WI-702 forbidden from touching the file; loc-guardian ignores this single unchanged known violation for WI-702; closes when the split WI lands. |
