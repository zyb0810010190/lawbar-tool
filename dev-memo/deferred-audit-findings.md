...
| PRC-4-FU | Medium | Open — sibling hooks `block-run-control-bash-write.sh` `emit_deny()` (line ~29) and `batch-commit-guard.sh` `deny()` (line ~19) build their `permissionDecisionReason` JSON with the SAME sed-only escaping (`\`/`"` only, no C0 control bytes) that PRC-4 fixed in `protect-run-control.sh`. Same fail-open robustness gap if a control byte ever reaches their reason. NOT fixed under PRC-4 (scope was `protect-run-control.sh` `deny()` only; "do not touch unrelated hook behavior"). Both reasons today are fixed strings / `$(basename …)` so not known reachable, like PRC-4. | WI: deny-json-parity (apply the PRC-4 jq-Rs + fallback pattern to the two sibling hooks) | YES | open | port the PRC-4 `deny()` body verbatim; one small WI when breaker headroom allows |
