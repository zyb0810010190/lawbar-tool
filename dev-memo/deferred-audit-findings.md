...
| FACTS-AUD-3 | Low | Accepted-as-intentional (consistent + defense-in-depth present) — `listFactsHandler` (`apps/lawbar-desktop/src/caseBox/factHandlers.ts:75`) returns the raw `ListFactsPage`, so `CaseBoxFact`'s server-authority fields (`tenant_id`, `actor_user_id`, `reviewer_actor_user_id`) cross the IPC boundary even though the renderer displays a subset. This is the SAME `return { ok: true, value: page }` pattern as `listDocumentsHandler` + `listDeadlinesHandler` (both audited CLEAN), and `renderer/api.ts` already strips to `RENDERER_LIST_FACTS_DTO_FIELDS` (§3 server-authority defense-in-depth). In single-user/single-tenant v1 the fields are the user's own — no second party to leak to. Surfaced by audit `audit-mpxpsc81-vsayi3`. | WI: handler-side DTO projection for all list channels (cross-cutting; project page→renderer-safe DTO in main process; do facts/documents/deadlines together, not facts alone) | YES (v1 single-tenant/single-user; renderer-side strip in place) | open | Auditor fix = map rows to a renderer-safe fact DTO in the handler. Deferred because applying it to facts alone diverges from the established sibling pattern; the right fix is one cross-cutting WI over all list handlers. Revisit before multi-tenant. |
