...
| BRCBW-7 | Low | Closed — three parts: (a) the `==`/`!=`/`=`/`test` co-occurrence forms already allow via the BRCBW-5 command-word gating (`[[`/`[`/`test` are not write verbs), proven by regression; (b) the literal `[[ x > config ]]` case (where pass-1 read `>` as redirection) is fixed by blanking `[[ … ]]` spans before pass-1 redirection detection — bash never redirects from inside `[[ ]]`, and a redirect after `]]` stays outside the span and is still denied; (c) the `[[ ]]` blank could have HIDDEN a real write executed via command substitution inside the test (`[[ "$(echo X > config)" == "" ]]`, `tee`, backticks). Closed by a new ADDITIVE pass-4 that extracts each `$(...)`/`` `…` `` body and runs the same redirection + write-verb detection on it (no `[[ ]]` blank) — it can only ADD denials, so no write protection is weakened; cmd-subs that only READ a protected path still allow. | WI: shell-token parsing | YES | closed (commit on `workflow-brcbw7-test-expr`; tests in `.claude/hooks/tests/block-run-control-bash-write.test.sh`) | ignore `[[ … ]]` contexts |
