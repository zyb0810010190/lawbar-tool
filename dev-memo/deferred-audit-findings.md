...
| CBW-UI-701-DEFAULT-PURPOSE | Low | Open (follow-up = WI-703) — WI-701 Layer-B closeout audit `audit-mq0zsm14-ic09t5` (BATCH-PASS C0 H0 M0 L1) found a real default-value correctness bug the per-WI MOCK tests missed: `FACT_PURPOSES` in `apps/lawbar-desktop/renderer/screens/viewMatterFacts.ts` lists `"claim"` first and no `<option>` is marked `selected`, so in a REAL browser an unchanged Add-Fact `<select>` defaults to its first option and would forward `purpose: "claim"` instead of the design's intended `"other"`. The mock test missed it because `MockEl.value` starts `undefined` (the handler's `\|\| "other"` fallback fires in the mock but not in a real browser, where `value` resolves to the first option). **Bounded impact**: the server still creates a VALID candidate fact (`"claim"` is a legal purpose) — no security / tenant-isolation / data-integrity effect; only the default purpose label is wrong when the user does not pick one. Fix requires a new WI because WI-701 is merged + closed. | WI-703 (BATCH-CASEBOX-UI-00; `wi-703-fix-default-purpose`): mark the `"other"` `<option selected>` (browser default = other) + a browser-faithful regression test asserting the untouched purpose select defaults to `other`, not the first option. | YES | open | non-blocking default-value bug; documented in `dev-memo/study/2026-06-05-batch-audit-54.md`; closes when WI-703 ships (status → closed, cite the WI-703 commit). |
