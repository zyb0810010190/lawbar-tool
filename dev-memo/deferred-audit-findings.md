...
| BRCBW-8 | Low | Open — nested command-substitution inner writes are not parsed. A write inside a NESTED substitution, e.g. `$(echo $(tee dev-memo/run/config))` or `` $(echo `tee config`) ``, can be missed by the Bash run-control guard: pass-4's `\$\([^)]*\)` / backtick extraction is non-recursive and stops at the first `)`, so only the outer body is scanned (its command word is the outer cmd, not the inner write). PRE-EXISTING on `main` (main has no pass-4 at all and also allows it); NOT introduced by PR #25 (commit `f880976`), which only added non-nested coverage. Remains an open guard-hardening gap. | WI-A: recursive/flatten substitution parsing (spec: `dev-memo/plan-brcbw-parser-depth-00.md` §3 Mechanism A) | YES | open | flatten `$(`/backtick/`)` delimiters then rescan, or recurse; keep additive (deny-only); design specified, not yet implemented |
