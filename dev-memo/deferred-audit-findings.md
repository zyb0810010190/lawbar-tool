...
| PRC-4-FU2 | Medium | Open — surfaced by the PRC-4-FU audit (`audit-mpzp0vaz-bgvs2s` Q7): TWO MORE deny emitters carry the same sed-only escaping (`\`/`"` only, no C0 control bytes) → same fail-open-on-invalid-JSON robustness gap: `block-git-add-all.sh` `emit_deny()` (line ~20) and `block-commit-stage-all.sh` `emit_deny()` (line ~19). These are the staging-hygiene commit hooks, NOT run-control authority emitters, so they were not in the PRC-4 / PRC-4-FU ledger scope. Same honest reachability posture (reasons are fixed strings / parsed git command echoes; not known reachable). | WI: deny-json-parity-2 (port the PRC-4 jq-Rs + fallback pattern to the two staging-hygiene hooks) | YES | open | port the PRC-4 `emit_deny()` body; one small WI when breaker headroom allows. After this, all five PreToolUse deny emitters share the hardened pattern. |
